...
# Constant parameters:
R = 155e-3 # Radius of the base in m
r = 80e-3  # Radius of the mobile platform in m
...
d = 250e-3  # Arm length (distance between the centers of the pivot joints) in mm
e = 415e-3  # Forearm length (distance from the pivot center arm-cross joint) in mm

phi0 = 0 # Initial angular position of the Servos 
z0 = 331e-3  # Height of the position when the arms are at 90°
zmax = 665e-3  # Height at the highest position (Servo at 0°)
zmin = 250e-3  # Height at the lowest position (Servo at 180°)
roll_min = -45
roll_max = 45
pitch_min = -45
pitch_max = 45
...
# Function calculating the angle value to send to the motor if it is not position controlled, based on Valeur Li
...
def AngleLi_MX_64(Li):
    Edeg = ((Li**2)+(d**2)-(e**2))/(2*d*Li)
    Edeg = np.clip(Edeg, -1, 1)
    xi = abs(-np.arccos(Edeg) + np.radians(phi0)) # Removed the 90° compared to Microgoat team's calculation because our O is vertical, and added absolute value as MX-64 is controlled within 0° to 180°
...
def animation(roll_start, pitch_start, height_start, roll_end, pitch_end, height_end, steps=100):
    ...
    def update(frame):
        ...
        for i in range(3):
            ...
            # Calculating the angular position to send to Servo i
            xi.append(AngleLi_MX_64(L[i]))
    
            # Limiting the extreme position of Servo i
            if xi[i]< xi_min:
                xi[i] = xi_min
            elif xi[i] > xi_max:
                xi[i] = xi_max
        
            # Displaying the angular position of Servo i
            print(f'xi{i+1} : ', xi[i])
            ...
            # Standard configuration (pivot of the arms contained within the volume of the 3DoF platform)
            ai = np.array([
            ...
            # 'Triangular' configuration: arms tangent to the base, only functional if r=R
            # ai = np.array([
    ...
    animation_result = 'P-3DoF Servos Animation Result'
    if not os.path.exists(animation_result):
        os.makedirs(animation_result)
    
    # Path to save the generated animation GIF
    gif_path = os.path.join(animation_result, 'P-3DoF Servos Animation.gif')
