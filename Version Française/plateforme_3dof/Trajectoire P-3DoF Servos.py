...
phi0 = 0 # Position angulaire initiale des Dynamixels 
z0 = 64.814 # Hauteur de la position lorsque les bras sont 90°
...
def trajectoire_servo(roulis_amp, tangage_amp, hauteur, steps=100):
    ...
    def update(frame):
        ...
        # Calcul des paramètres de roulis et tangage en fonction de l'itération
        roulis = roulis_amp * np.sin(np.radians(frame))
        tangage = tangage_amp * np.cos(np.radians(frame))
        hauteur_actuelle = (hauteur-50)+50*frame/360
        ...
        info_text = (f'\n'
                     ...
                     f'\n'
                     f'  Hauteur : {hauteur_actuelle:.4f} mm\n'
                     f'\n'
                     f'  Longueur L1 : {L[0]:.4f} mm\n'
                     f'\n'
                     f'  Longueur L2 : {L[1]:.4f} mm\n'
                     f'\n'
                     f'  Longueur L3 : {L[2]:.4f} mm\n'
                     f'\n'
                     f'  Angle xi1 : {xi[0]:.2f} °\n'
                     f'\n'
                     f'  Angle xi2 : {xi[1]:.2f} °\n'
    ...
    resultat_animation = 'Résultat trajectoire P-3DoF Servos'
    ...
    gif_path = os.path.join(resultat_animation, 'Trajectoire P-3DoF Servos.gif')
...
trajectoire_servo(roulis_amp=10, tangage_amp=10, hauteur=100, steps=360)
